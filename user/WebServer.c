...
static const char pageMQTTConf[]  = "<html><body><form action=\"http://192.168.4.1/mqttconfig\" method=\"post\">\
<p>MQTT Server Addr:<input type=\"text\" name=\"serv\" />\
<p>MQTT Server Port:<input type=\"text\" name=\"port\" /></p>\
<p>MQTT UserName:<input type=\"text\" name=\"user\" /></p>\
<p>MQTT Password:<input type=\"text\" name=\"passwd\" /></p>\
<p>Client ID:<input type=\"text\" name=\"clientid\" /></p>\
<input type=\"submit\" value=\"Submit\" /></form>\
<button type=\"skip\" onclick=\"window.location.href='http://192.168.4.1/serverconfig'\">Skip to Server config</button>\
</body></html>";
static const char pageServConf[]  = "<html><body><form action=\"http://192.168.4.1/serverconfig\" method=\"post\">\
<p>Server Addr:<input type=\"text\" name=\"serv\" />\
Use Domain<input type=\"checkbox\" name=\"domain\"></p>\
<p>Server Port:<input type=\"text\" name=\"port\" /></p>\
<input type=\"submit\" value=\"Submit\" />\
</form></body></html>";
...
static const char pageRESET[]  = "<html><body><form action=\"http://192.168.4.1/reset\" method=\"post\">\
<input type=\"hidden\" name=\"reset\" value=\"now\" />\
...
WebServTask(os_event_t *e){
	...
	case WSIG_WIFI_CHANGE:
		...
		break;
	case WSIG_MQTT_CONF:
		system_os_post(TCPCOMM_TASK_PRIO,TSIG_MQTT_CONF,0x00);
		break;
	case WSIG_REMOTE_CONF:
...
WebServRecvCB(void *arg, char *pDataRecv, unsigned short length)
{
	...
	if(pUrlParam->eMethod == GET){
		if((pUrlParam->szPath[0]==0)||(os_strncmp(pUrlParam->szPath,pathWifiConf,sizeof(pathWifiConf))==0) ){
			...
			respCode = SUCCESS;
			goto EXIT_FREE_URLPARAM;
		}else if(os_strncmp(pUrlParam->szPath,pathMQTTConf,sizeof(pathMQTTConf))==0){
			pRespBuf = (char*)pageMQTTConf;
			respCode = SUCCESS;
			goto EXIT_FREE_URLPARAM;
			...
			respCode = SUCCESS;
			goto EXIT_FREE_URLPARAM;
		}else if(os_strncmp(pUrlParam->szPath,pathRESET,sizeof(pathRESET))==0){
			pRespBuf = (char*)pageRESET;
			respCode = SUCCESS;
			goto EXIT_FREE_URLPARAM;
		}
		respCode = BAD_REQUEST;
		...
		if(os_strncmp(pUrlParam->szPath,pathWifiConf,sizeof(pathWifiConf))==0){
			...
			cJSON *MQTTServ = cJSON_GetObjectItem(pPostParam->jsonData,"serv");
			cJSON *MQTTPort = cJSON_GetObjectItem(pPostParam->jsonData,"port");
			cJSON *ClientID = cJSON_GetObjectItem(pPostParam->jsonData,"clientid");
			cJSON *UserName = cJSON_GetObjectItem(pPostParam->jsonData,"user");
			cJSON *Password = cJSON_GetObjectItem(pPostParam->jsonData,"passwd");

			if(MQTTServ==NULL || os_strlen(MQTTServ->valuestring)>DOMAIN_LEN
					|| MQTTPort==NULL || os_strlen(MQTTPort->valuestring)>PORT_NUM_LEN
					|| ClientID==NULL || os_strlen(ClientID->valuestring)>MQTT_CLIENT_ID_LEN
					|| UserName==NULL || os_strlen(UserName->valuestring)>MQTT_USER_NAME_LEN
					|| Password==NULL  || os_strlen(Password->valuestring)>MQTT_PASSWORD_LEN ){
				respCode = BAD_REQUEST;
				pRedirect = pathMQTTConf;
				goto EXIT_FREE_JSONPARAM;
			}

			TRACE("mqtt server:%s\r\n",MQTTServ->valuestring);
			os_memset(stFlashProtParam.MQTTServAddr,0,sizeof(stFlashProtParam.MQTTServAddr));
			os_memcpy(stFlashProtParam.MQTTServAddr,MQTTServ->valuestring,os_strlen(MQTTServ->valuestring));

			TRACE("port:%s\r\n",MQTTPort->valuestring);
			stFlashProtParam.MQTTServPort = atoi(MQTTPort->valuestring);

			TRACE("client id:%s\r\n",ClientID->valuestring);
			os_memset(stFlashProtParam.MQTTClientID,0,sizeof(stFlashProtParam.MQTTClientID));
			os_memcpy(stFlashProtParam.MQTTClientID,ClientID->valuestring,os_strlen(ClientID->valuestring));

			TRACE("client user:%s\r\n",UserName->valuestring);
			os_memset(stFlashProtParam.MQTTUserName,0,sizeof(stFlashProtParam.MQTTUserName));
			os_memcpy(stFlashProtParam.MQTTUserName,UserName->valuestring,os_strlen(UserName->valuestring));

			TRACE("client password:%s\r\n",Password->valuestring);
			os_memset(stFlashProtParam.MQTTPassword,0,sizeof(stFlashProtParam.MQTTPassword));
			os_memcpy(stFlashProtParam.MQTTPassword,Password->valuestring,os_strlen(Password->valuestring));

			stFlashProtParam.WorkStatus |= MQTT_CLIENT_BIT | MQTT_CONF_BIT;
			SaveFlashProtParam();

			system_os_post(WEBSERV_TASK_PRIO,WSIG_MQTT_CONF,0x00);
			...
			system_os_post(WEBSERV_TASK_PRIO,WSIG_REMOTE_CONF,0x00);

			respCode = SUCCESS;
			pRespBuf = (char*)pageConfFinish;
			goto EXIT_FREE_JSONPARAM;
		}else if(os_strncmp(pUrlParam->szPath,pathRESET,sizeof(pathRESET))==0){
			cJSON *reset = cJSON_GetObjectItem(pPostParam->jsonData,"reset");
			if(reset==NULL){
				respCode = BAD_REQUEST;
				pRedirect = pathServConf;
				goto EXIT_FREE_JSONPARAM;
			...
			TRACE("server:%s\r\n",reset->valuestring);
			if(os_strncmp(reset->valuestring,"true",4)==0){
				stFlashProtParam.WorkStatus = WEB_SERV_BIT;
				SaveFlashProtParam();
				system_restart();
			}else{
				respCode = BAD_REQUEST;
				pRedirect = pageWifiConf;
				goto EXIT_FREE_JSONPARAM;
			}
		}else{
