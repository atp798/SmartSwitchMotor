...
LoadFlashProtParam(){
	...
	if(stFlashProtParam.ConfHolder != CONF_HOLDER){
		...
		stFlashProtParam.WorkStatus = WEB_SERV_BIT;
		memset(stFlashProtParam.AlarmQueue,0,sizeof(stFlashProtParam.AlarmQueue));
	...
	TRACE("load flash protected param:\r\nWork Status:%02x\r\n",stFlashProtParam.WorkStatus);
	...
	uint8 MQTTServAddr[DOMAIN_LEN+1];
	uint16 MQTTServPort;
	uint8 MQTTClientID[MQTT_CLIENT_ID_LEN+1];
	uint8 MQTTUserName[MQTT_USER_NAME_LEN+1];
	uint8 MQTTPassword[MQTT_PASSWORD_LEN+1];
	uint32 ReportInterval; //unit milliseconds

	TRACE("MQTT server address:%s:%d\r\n",stFlashProtParam.MQTTServAddr,stFlashProtParam.MQTTServPort);
	TRACE("MQTT client ID:%s\r\n",stFlashProtParam.MQTTClientID);
	TRACE("MQTT user %s:%s\r\n",stFlashProtParam.MQTTUserName,stFlashProtParam.MQTTPassword);
